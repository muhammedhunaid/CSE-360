...
public class CreateAccountController extends SceneController implements Initializable{
    ...
    User currentUser = App.getAppUser();
    ...
    void join(ActionEvent event) throws IOException, InterruptedException, SQLException {
        ...
        if(valid_pw && valid_uname && same) {
            ...
            if(App.setting_up_admin) {
                App.databaseHelper.addUser(username, password, "admin"); // Add as admin
                App.setting_up_admin = false; // Switch off admin setup
            } else {
            ...
            pause.setOnFinished(ActionEvent -> {
                try {
                    App.setRoot("login"); // Switch to the login screen after the pause
                } catch (IOException e) {
