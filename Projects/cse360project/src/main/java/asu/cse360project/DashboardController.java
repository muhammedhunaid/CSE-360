...
public class DashboardController extends SceneController implements Initializable {
    ...
    void logout(ActionEvent event) {
        ...
        alert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                ...
                try {
                    App.setAppUser(null); // Clear the current user
                    App.setRoot("login"); // Load the login scene
                } catch (IOException e) {
    ...
    // Load the specified FXML file and set it as the content of the content area
    private void setContentArea(String fxml) throws IOException {
        ...
        contentArea.getChildren().removeAll(); // Remove existing children from content area
        contentArea.getChildren().setAll(fxmlLoader); // Set the new scene
    }
}
