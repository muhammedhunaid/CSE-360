...
public class NewPwController implements Initializable {
    ...
    private User curr_user = App.getAppUser();
    ...
    void Finish(ActionEvent event) throws SQLException, IOException {
        ...
        if (valid_pw && same) {
            ...
            App.setRoot("login"); // Navigate to the login scene
            App.setAppUser(null); // Clear the current user from the application context
        }
    }
}
